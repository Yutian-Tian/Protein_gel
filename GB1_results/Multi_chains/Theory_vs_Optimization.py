...
def Plot2state(r_fold, r_unfold, f, lineType):
    ...
    Et = single_domain_free_energy(r_unfold, 1.0, E_mean) - single_domain_free_energy(r_fold, 0.0, E_mean) 
    print(Et)
    p_u = 1 - 1/(1 + np.exp(-Et - f * (r_fold - r_unfold)))
    line = plt.plot(f, 10*p_u, lineType, color='blue', 
                    linewidth=lines_linewidth, label=f'$\Delta E_t = {Et:.1f}$, $r_s = {r_unfold - r_fold:.1f}$', zorder=3)
