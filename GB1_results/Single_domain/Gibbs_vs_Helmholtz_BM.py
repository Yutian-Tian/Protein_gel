...
# ======================================================
# 【新增改动】：将以下两个函数替换为图2的WLC新公式
# 图2公式: F/(k_B T) = π² c (1 - x²) + 1 / (π c (1 - x²))
# 其中 c = lp / (2 Lc)，取 lp = 1, k_B T = 1
# ======================================================
def F_MS(r, n):
    """Marko-Siggia WLC自由能 (更新为图2对应公式)"""
    Lc_val = Lc(n)
    x_val = r / Lc_val
    # 数值稳定，防止 x 过于接近 1
    ...
    # 对应图2公式进行计算
    c = 1.0 / (2.0 * Lc_val)
    return pi**2 * c * (1.0 - x_clipped**2) + 1.0 / (pi * c * (1.0 - x_clipped**2))

def force_MS(r, n):
    """Marko-Siggia 力-距关系 (由新公式求导得出)"""
    Lc_val = Lc(n)
    x_val = r / Lc_val
    x_clipped = np.clip(x_val, 0.001, 0.999)
    
    # 对应的解析导数公式
    term1 = - (pi**2 * x_clipped) / (Lc_val**2)
    term2 = (4.0 * x_clipped) / (pi * (1.0 - x_clipped**2)**2)
    return term1 + term2
# ======================================================
...
def main():
    ...
    # 保存图片：提前创建目录以防止文件错误
    save_path = '/home/tyt/project/Single-chain/GB1_results/Single_domain/Gibbs_vs_Helmholtz_fr_BM'
