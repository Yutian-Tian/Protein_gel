"""
使用了半刚性链模型（WLC）优化计算，绘制了n-r和f-r曲线。
"""
...
def WLC_free_energy(x, Lc_val):
    """
    WLC自由能（更新为图2形式）
    对应图中公式: F/(k_B T) = π² c (1 - x²) + 1 / (π c (1 - x²))
    其中 c = lp / (2 Lc)
    """
    if x >= 1.0 - 1e-12:
    ...
    c = lp / (2.0 * Lc_val)
    return np.pi**2 * c * (1.0 - x**2) + 1.0 / (np.pi * c * (1.0 - x**2))
...
def MS_force(r, Lc_val):
    """
    Marko-Siggia 力 (由新自由能求导): f = dF/dr
    对应新公式解析导数: 
    f = - π² * (lp / Lc²) * x + (4 / (π lp)) * x / (1 - x²)²
    """
    x = np.asarray(r, dtype=np.float64) / np.asarray(Lc_val, dtype=np.float64)
    force = np.where(x < 1.0 - 1e-12,
                     - np.pi**2 * lp * x / (Lc_val**2) + 4.0 * x / (np.pi * lp * (1.0 - x**2)**2),
                     1e15)
    return force
...
def main():
    ...
    # 保存图形
    base_save_path = '/home/tyt/project/protein_gel/opt+R/Single_domain/simulation_results/theory_vs_numerical_BM'
